...
import { getFirestore, collection, getDocs, query, where, doc, deleteDoc } from "https://www.gstatic.com/firebasejs/11.0.1/firebase-firestore.js";
...
async function fetchModules() {
  ...
  const q = query(modulesRef, where("fld_userId", "==", loggedInUserId));
  ...
  querySnapshot.forEach((doc) => {
    ...
    moduleItem.innerHTML = `
      <a href="#../assets/modules/${fld_language}/${fld_fileName}" target="_blank">
        <img src="../assets/images/pdf icon.png" alt="PDF Icon">
        <span class="module-name">${fld_name}</span>
      </a>
      <span class="dots">...</span>
      <div class="options" style="display: none;">
        <button class="view" id="view-${moduleId}">View</button>
  ...
  document.getElementById("total-modules").innerHTML = `Total Modules Posted<br>${count}`;
}
